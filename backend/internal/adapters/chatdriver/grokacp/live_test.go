...
// turn. The contract tests assert the exact argv and session mode string; this
// live matrix catches provider-side drift in how Grok honours them. Run
// explicitly because it consumes account turns and may surface real prompts.
func TestLiveGrokACPPermissionModes(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			// The edit tool, not the shell, is what accept-edits scopes, so the
			// prompt must exercise it for the modes to differ at all.
			...
			// Only default mode is expected to prompt; approving keeps the
			// matrix from failing on a mode that legitimately asks.
